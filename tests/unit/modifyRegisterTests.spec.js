...
describe("Modify Delete Income Expense", () => {
  ...
  it("Update", () => {
    ...
    assert.equal(wrapper1.vm.$store.state.TRANSACTIONS[21].name, "example");
    ...
    assert.equal(wrapper1.vm.$store.state.TRANSACTIONS[21].name, "20");
  });
});
describe("Register Income Expense", () => {
  ...
  it("increment when save an item", () => {
    ...
    wrapper.vm.date = "2000-12-3";
    wrapper.vm.registerItem();
    assert.equal(wrapper.vm.$store.state.TRANSACTIONS.length, 23);
  });
  it("find id category", () => {
    const localVue = createLocalVue();
    localVue.use(VueRouter);
